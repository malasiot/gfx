...
float Context::toPixels(const Length &l, LengthDirection dir, bool scale_to_viewport) {
    double factor = 1.0 ;
...
void RenderingContext::preRenderShape(Element &e, const Style &s, const Matrix2d &t, const gfx::Rectangle2d &bounds)
{
    pushState(s) ;
    pushTransform(t) ;
...
void RenderingContext::setPaint(Element &e)
{
    ...
    if ( fill_paint.type() == PaintType::SolidColor ||
        ...
        if ( fill_rule == FillRule::NonZero )
            brush.setFillRule(gfx::FillRule::NonZero) ;
        else
            brush.setFillRule(gfx::FillRule::EvenOdd) ;
...
void RenderingContext::applyClipPath(ClipPathElement *cp)
{
    ...
    RenderingContext clipCtx(clip_canvas, fw, fh, RenderingMode::Cliping) ;
    clipCtx.obbox_ = obbox_ ;
    ...
    if ( cp->style().getClipRule() == ClipRule::EvenOdd )
        fr = gfx::FillRule::EvenOdd ;
    else
        fr = gfx::FillRule::NonZero ;
...
Font RenderingContext::makeFont(const Style &st)
{
    ...
    case FontStyle::Oblique:
        f.setStyle(gfx::FontStyle::Oblique) ;
        break ;
    case FontStyle::Italic:
        f.setStyle(gfx::FontStyle::Italic) ;
        break ;
    case FontStyle::Normal:
        f.setStyle(gfx::FontStyle::Normal) ;
        break ;
    }
    ...
    case FontWeight::W400:
        f.setWeight(gfx::FontWeight::Normal) ;
        break ;
    ...
    case FontWeight::W900:
        f.setWeight(gfx::FontWeight::Bold) ;
        break ;
    }
...
void RenderingContext::render(SVGElement &e) {
  ...
  //  if ( doc_width_hint_ != 0 ) sw = doc_width_hint_ ;
  //  if ( doc_height_hint_ != 0 ) sh = doc_height_hint_ ;
  ...
  //  setOverflow(e.style(), Rectangle2d(xx, yy, sw, sh)) ;
    ...
    setOverflow(e.style(), Rectangle2d(vbox.x_, vbox.y_, vbox.width_, vbox.height_)) ;
