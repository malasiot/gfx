#ifndef GFX_SVG_STYLE_HPP
#define GFX_SVG_STYLE_HPP
...
public:
    ...
    SVG_STYLE_ATTRIBUTE(StopColor,  gfx::CSSColor, stop_color_, gfx::NamedColor::black())
...
private:
    ...
    template<typename T>
    void parseAttribute(const std::string &name, const std::string &val, std::optional<T> &a) {
        T tmp ;
        if ( !tmp.parse(val) )
...
template<>
void Style::parseAttribute(const std::string &name, const std::string &val, std::optional<CSSColor> &a) ;
