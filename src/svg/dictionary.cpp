...
Dictionary Dictionary::decode(const char *data, size_t sz, const char tag_delimeter) {
    Dictionary tags ;

    const char *c = data ;
    while ( *c != tag_delimeter) {
        string key, val ;
        while ( *c != tag_delimeter ) key += *c++ ;
        ++c ;
        while ( *c != tag_delimeter ) val += *c++ ;
        ++c ;
        tags.add(key, val) ;
    }
    return tags ;
...
string Dictionary::encode(const char tag_delimeter) const {
    string res ;
