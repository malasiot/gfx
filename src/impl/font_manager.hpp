#ifndef GFX_FONT_MANAGER_HPP
#define GFX_FONT_MANAGER_HPP
...
public:
    ...
    cairo_font_face_t *queryFace(const std::string &family_name, gfx::FontStyle font_style, gfx::FontWeight font_weight) ;

    // Calls query face and if sucessfull scales the font found
    cairo_scaled_font_t *createFont(const gfx::Font &font) ;

private:

    FontManager() = default ;

    static std::string font_face_key(const std::string &family_name, gfx::FontStyle font_style, gfx::FontWeight font_weight) ;
